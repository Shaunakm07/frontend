...
function ResearchFigure({ src, alt, label }: { src: string; alt: string; label: string }) {
  return (
    <div className="relative overflow-hidden aspect-[4/3] bg-[#06060a] border-r border-[rgba(240,237,232,0.07)] max-[680px]:border-r-0 max-[680px]:border-b group">
      <img src={src} alt={alt} className="w-full h-full object-cover opacity-[0.92] group-hover:opacity-100 transition-opacity duration-300" />
      <span className="absolute bottom-3 left-3.5 text-[10px] font-medium tracking-[0.05em] uppercase text-[rgba(240,237,232,0.4)] bg-[rgba(6,6,10,0.8)] px-2 py-[3px]">
        {label}
      </span>
    </div>
  )
...
function StripFigure({ src, alt, caption }: { src: string; alt: string; caption: string }) {
  return (
    <div className="relative overflow-hidden aspect-video bg-[#06060a] border-r border-[rgba(240,237,232,0.07)] last:border-r-0 max-[520px]:border-r-0 max-[520px]:border-b group">
      <img src={src} alt={alt} className="w-full h-full object-cover opacity-80 group-hover:opacity-100 transition-opacity duration-200" />
      <div className="absolute bottom-0 left-0 right-0 px-3.5 pb-2.5 pt-5" style={{ background: 'linear-gradient(transparent, rgba(6,6,10,0.85))' }}>
        <p className="text-[10px] text-[rgba(240,237,232,0.4)] tracking-[0.03em]">{caption}</p>
      </div>
    </div>
  )
}
...
export default function Home() {
  return (
    <>
      ...
      <main>
        {/* ── Hero ── */}
        <section className="relative min-h-screen flex items-center overflow-hidden" id="waitlist">
          <BrainCanvas variant="hero" />
          <div className="relative z-10 max-w-[520px] w-full px-[clamp(20px,6vw,80px)] pt-[100px] pb-[80px]">
            <p className="flex items-center gap-2 text-[11px] font-medium tracking-[0.08em] uppercase text-white/50 mb-9">
              <span className="w-[5px] h-[5px] rounded-full bg-[#f03d78] flex-shrink-0 animate-pulse" />
              Emotion intelligence · Private beta
            </p>
            <h1 className="text-[clamp(48px,7.5vw,80px)] font-bold leading-[1.04] tracking-[-0.035em] mb-7">
              Know what your images<br />
              <span className="grad-text">do to the brain.</span>
            </h1>
            <p className="text-[17px] font-light text-white/80 leading-[1.7] max-w-[460px] mb-11">
        ...
        <div className="max-w-[calc(840px+clamp(20px,6vw,80px)*2)] mx-auto px-[clamp(20px,6vw,80px)]">
          <section className="pt-[100px]">
            <p className="text-[10px] font-semibold tracking-[0.1em] uppercase text-white/50 mb-3.5">Experiment 1 · May 26, 2026</p>
            <h2 className="text-[clamp(24px,3.8vw,36px)] font-bold tracking-[-0.03em] leading-[1.12] mb-[18px] max-w-[560px]">
              We fine-tuned a language model using real brain signals. It worked.
            </h2>
            <p className="text-[14px] font-light text-white/80 leading-[1.75] max-w-[520px] mb-10">
            ...
            <div className="grid grid-cols-2 max-[680px]:grid-cols-1 border border-[rgba(240,237,232,0.07)]">
              <ResearchFigure src="/plots/fig3_brain_surface.png" alt="Cortical surface maps — base, LoRA, difference" label="Cortical surface — base · LoRA · Δ" />
              <div className="flex flex-col justify-center px-10 py-11 gap-7 max-[520px]:px-6">
                {[
                  ...
                  <div key={num}>
                    <div className="text-[42px] font-bold tracking-[-0.04em] leading-none mb-[5px]" style={{ background: grad, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>
                      {num}
                    </div>
                    <div className="text-[12px] text-white/50 tracking-[0.02em]">{label}</div>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="grid grid-cols-3 max-[520px]:grid-cols-1 border-t border-l border-r border-[rgba(240,237,232,0.07)]">
              <StripFigure src="/plots/fig5_training_trajectory.png" alt="Training trajectory" caption="Reward · loss · KL over 200 steps" />
              <StripFigure src="/plots/fig2_roi_bar_chart.png" alt="ROI bar chart" caption="20 cortical ROI activations — base vs LoRA" />
              <StripFigure src="/plots/fig1_all_roi_timeseries.png" alt="All ROI timeseries" caption="BOLD timeseries — all 20 regions" />
            </div>

            <div className="border border-[rgba(240,237,232,0.07)] px-8 py-7 flex items-center justify-between gap-5 flex-wrap">
              <p className="text-[14px] font-light text-white/80 leading-[1.6]">
              ...
              <Link href="/blog#brain-llm" className="inline-flex items-center gap-1.5 text-[12px] font-bold tracking-[0.04em] uppercase grad-text no-underline hover:opacity-75 transition-opacity whitespace-nowrap">
        ...
        <div className="max-w-[calc(840px+clamp(20px,6vw,80px)*2)] mx-auto px-[clamp(20px,6vw,80px)]">
          <section className="pt-16">
            <p className="text-[10px] font-semibold tracking-[0.1em] uppercase text-white/50 mb-3.5">Experiment 2 · May 29, 2026</p>
            <h2 className="text-[clamp(24px,3.8vw,36px)] font-bold tracking-[-0.03em] leading-[1.12] mb-[18px] max-w-[560px]">
            ...
            <div className="grid grid-cols-4 max-[680px]:grid-cols-2 border border-[rgba(240,237,232,0.07)]">
              {[
                ...
                { num: '0%', grad: 'linear-gradient(135deg,#b840b8,#6040e8)', label: 'MMLU regression in STEM and Humanities — factual knowledge intact' },
                { num: '5/5', grad: 'var(--grad)', label: 'Quality axes where LoRA leads — engagement, clarity, coherence, creativity, instruction following' },
              ].map(({ num, grad, label }) => (
                <div key={num} className="px-6 py-7 border-r border-[rgba(240,237,232,0.07)] last:border-r-0 max-[680px]:[&:nth-child(2)]:border-r-0 max-[680px]:[&:nth-child(3)]:border-r">
                  <div className="text-[42px] font-bold tracking-[-0.04em] leading-none mb-[5px]" style={{ background: grad, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>
                    {num}
                  </div>
                  <div className="text-[12px] text-white/50 tracking-[0.02em]">{label}</div>
            ...
            <div className="border border-[rgba(240,237,232,0.07)] border-t-0 px-8 py-7 flex items-center justify-between gap-5 flex-wrap mb-20">
              <p className="text-[14px] font-light text-white/80 leading-[1.6]">
              ...
              <Link href="/blog#eval-suite" className="inline-flex items-center gap-1.5 text-[12px] font-bold tracking-[0.04em] uppercase grad-text no-underline hover:opacity-75 transition-opacity whitespace-nowrap">
                Read the eval suite →
              </Link>
            </div>
          </section>
        ...
        <div className="max-w-[calc(840px+clamp(20px,6vw,80px)*2)] mx-auto px-[clamp(20px,6vw,80px)]">
          <section className="pb-0">
            <div className="flex items-end justify-between gap-5 mb-7 flex-wrap">
              <div>
                <p className="text-[10px] font-semibold tracking-[0.1em] uppercase text-white/50 mb-2.5">Progress Notes · Members only</p>
                <h2 className="text-[clamp(18px,2.6vw,24px)] font-bold tracking-[-0.025em]">What we&apos;re learning, in writing.</h2>
              </div>
              <Link href="/blog" className="inline-flex items-center gap-1.5 text-[12px] font-bold tracking-[0.04em] uppercase grad-text no-underline hover:opacity-75 transition-opacity whitespace-nowrap">
                Enter your email to read →
              </Link>
            </div>

            <div className="grid grid-cols-2 max-[600px]:grid-cols-1 gap-px bg-[rgba(240,237,232,0.07)] border border-[rgba(240,237,232,0.07)] mb-20">
              {blogPreviews.map(({ date, tag, featured, title, excerpt, href }) => (
                <article key={title} className="bg-[#09090d] px-7 pt-7 pb-6 flex flex-col gap-2.5 hover:bg-[rgba(240,237,232,0.017)] transition-colors duration-150">
                  <div className="flex items-center gap-2.5">
                    <span className="text-[10px] text-white/50 tracking-[0.03em]">{date}</span>
                    <span
                      className="text-[9.5px] font-semibold tracking-[0.06em] uppercase px-[7px] py-0.5 border border-[rgba(240,237,232,0.07)] text-white/50"
                      style={featured ? { color: 'rgba(240,62,120,0.7)', borderColor: 'rgba(240,62,120,0.18)' } : {}}
                    >
                      {tag}
                    </span>
                  </div>
                  <h3 className="text-[14px] font-semibold tracking-[-0.01em] leading-[1.3] text-[#f0ede8]">{title}</h3>
                  <p className="text-[12.5px] font-light text-white/50 leading-[1.7] line-clamp-2">{excerpt}</p>
                  <div className="flex items-center gap-1.5 mt-1 text-[10.5px] text-white/50 tracking-[0.02em]">
                    <span className="inline-block w-2.5 h-3 border border-[rgba(240,237,232,0.25)] rounded-sm" />
                    Members only ·{' '}
                    <Link href={href} className="no-underline hover:text-[rgba(240,62,120,0.9)] transition-colors" style={{ color: 'rgba(240,62,120,0.65)' }}>
                      Enter your email to read →
                    </Link>
                  </div>
                </article>
        ...
        {/* ── Features ── */}
        <div className="max-w-[calc(840px+clamp(20px,6vw,80px)*2)] mx-auto px-[clamp(20px,6vw,80px)]">
          <section className="border-t border-[rgba(240,237,232,0.07)] pb-[120px]" id="about">
            {[
              ...
              <div key={n} className="grid grid-cols-[40px_1fr] gap-x-7 py-11 border-b border-[rgba(240,237,232,0.07)] last:border-b-0">
                <span className="text-[11px] font-medium text-white/50 tracking-[0.04em] pt-[3px]">{n}</span>
                <div>
                  <h3 className="text-[16px] font-semibold tracking-[-0.01em] mb-2">{title}</h3>
                  <p className="text-[14px] text-white/80 font-light leading-[1.75] max-w-[540px]">{body}</p>
                </div>
              </div>
            ))}
          </section>
